...
class CloudbedsClient {
   ...
   * @param {string} options.roomTypeID - Room type ID (required for initial assignment)
   * @param {string} options.oldRoomID - Old room ID (required for reassignment)
   * @returns {Promise<Object>} Assignment result
   */
  async assignRoom(reservationId, roomId, options = {}) {
    const params = {
      reservationID: reservationId,
      newRoomID: roomId,
      propertyID: this.propertyId,
    ...
    // Add roomTypeID or oldRoomID (one is required by Cloudbeds)
    if (options.roomTypeID) {
      params.roomTypeID = options.roomTypeID;
    }
    if (options.oldRoomID) {
      params.oldRoomID = options.oldRoomID;
    }
    
    return await this.makeRequest('/postRoomAssign', 'POST', params);
  }
